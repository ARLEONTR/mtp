...
extern int      MTP_dointvec(struct ctl_table *table, int write, void __user *buffer, size_t *lenp, loff_t *ppos);
extern int      MTP_err_handler_v4(struct sk_buff *skb, u32 info);
extern int      MTP_err_handler_v6(struct sk_buff *skb, struct inet6_skb_parm *, u8,  u8,  int,  __be32);
extern int      MTP_get_port(struct sock *sk, unsigned short snum);
extern int      MTP_getsockopt(struct sock *sk, int level, int optname, char __user *optval, int __user *option);
extern int      MTP_ioctl(struct sock *sk, int cmd, int * arg);
extern int      MTP_sendmsg(struct sock *sk, struct msghdr *msg, size_t len);
extern int      MTP_setsockopt(struct sock *sk, int level, int optname, sockptr_t __user optval, unsigned int optlen);
...
extern __poll_t MTP_poll(struct file *file, struct socket *sock, struct poll_table_struct *wait);
extern int      MTP_recvmsg(struct sock *sk, struct msghdr *msg, size_t len,int flags, int *addr_len);
...
extern ssize_t  MTP_metrics_read(struct file *file, char __user *buffer,size_t length, loff_t *offset);
