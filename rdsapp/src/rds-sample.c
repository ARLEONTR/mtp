...
static void create_message(char *buf, uint32_t start)
{
	...
	for (i = 0; i < BUFSIZE; i++)
		buf[i] = ((i + start) % NUM_PRINTABLE_CHARS) +
			 PRINTABLE_CHARS_OFFSET;
}
	...
	do {
		...
		if (flags & VERBOSE_FLAG)
			printf("Received %s packet %d of len %d, cmsg len %d, on port %d\n",
			       msg.msg_controllen ? "RDS RDMA" : "RDS", count,
			       (uint32_t)iov[0].iov_len,
			       (uint32_t)msg.msg_controllen, din.sin_port);
...
static void client(char *localaddr, char *remoteaddr, uint32_t flags, int count)
{
	...
	for (num_mess = count; num_mess || count == -1; num_mess--) {
		...
		if (flags & RDMA_READ_FLAG || flags & RDMA_WRITE_FLAG) {
			rc = build_rds_rdma_packet(sock, &msg, &mess, &cookie,
						   &flags);
			if (rc < 0)
				goto out;
...
int oldmain(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp("-s", argv[i]) || !strcmp("--server", argv[i])) {
			...
			count = atoi(argv[i + 1]);
			i++;
		} else if (!strcmp("-rr", argv[i]) ||
			   !strcmp("--rdma-read", argv[i])) {
			flags |= RDMA_READ_FLAG;
		} else if (!strcmp("-rw", argv[i]) ||
			   !strcmp("--rdma-write", argv[i])) {
...
char *serveraddr = "127.0.0.1", *clientaddr = "127.0.0.1";
uint32_t flags = 0;
...
{
	client(clientaddr, serveraddr, flags, 100);
	return NULL;
}
...
int main(int argc, char **argv)
{
	...
	pthread_create(&thread_server, NULL, serverThread, NULL);
	pthread_create(&thread_client, NULL, clientThread, NULL);
